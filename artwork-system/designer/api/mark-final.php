...
try {
    ...
    if ($fileId <= 0) {
        $stmt = $db->prepare('SELECT id FROM artwork_files WHERE project_id = ? ORDER BY version DESC LIMIT 1');
    ...
    $stmt = $db->prepare('SELECT filename, original_name, version FROM artwork_files WHERE id = ? AND project_id = ?');
    ...
    $db->prepare('UPDATE artwork_files SET is_final = 0 WHERE project_id = ?')->execute([$projectId]);
