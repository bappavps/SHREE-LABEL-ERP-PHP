...
// Get Project
$stmt = $db->prepare("SELECT * FROM projects WHERE token = ?");
$stmt->execute([$token]);
$project = $stmt->fetch();
...
// Get Latest File
$stmt = $db->prepare("SELECT * FROM files WHERE project_id = ? ORDER BY version DESC LIMIT 1");
...
$stmt = $db->prepare("SELECT * FROM comments WHERE file_id = ? AND parent_id IS NULL ORDER BY created_at ASC");
$stmt->execute([$file['id']]);
$comments = $stmt->fetchAll();

// Full File Journey (for client/designer/admin visibility)
$stmt = $db->prepare("SELECT id, filename, original_name, file_type, version, uploaded_at, is_final FROM files WHERE project_id = ? ORDER BY version ASC");
$stmt->execute([$project['id']]);
$projectFiles = $stmt->fetchAll();

$stmt = $db->prepare("SELECT action, created_at FROM activity_log WHERE project_id = ? ORDER BY created_at DESC LIMIT 40");
$stmt->execute([$project['id']]);
$projectActivities = $stmt->fetchAll();

$stmt = $db->prepare("SELECT COUNT(*) FROM comments c JOIN files f ON f.id = c.file_id WHERE f.project_id = ?");
$stmt->execute([$project['id']]);
...
<body>
    <header class="review-header">
        <div style="display: flex; align-items: center; gap: 1.5rem;">
            <div class="logo-text">
                <i class="fas fa-shapes"></i> Artwork Approval Hub
            </div>
    ...
    <div class="review-container">
        ...
        <aside class="history-sidebar" id="history-sidebar">
            ...
            <div class="project-history-panel">
                <p class="history-title">Project History (Visible to Client/Designer/Admin)</p>
                    ...
                    <?php else: ?>
                        <?php foreach ($projectFiles as $fileIndex => $journeyFile): ?>
                            ...
                            <div class="history-file-item<?php echo $isLatest ? ' active-version' : ''; ?>"
                                 onclick="switchVersion(<?php echo htmlspecialchars(json_encode(['id'=>(int)$journeyFile['id'],'filename'=>$journeyFile['filename'],'file_type'=>$journeyFile['file_type'] ?? 'image','version'=>(int)$journeyFile['version'],'is_latest'=>$isLatest]), ENT_QUOTES); ?>)"
                ...
                <div class="history-activity-list">
                    ...
                    <?php else: ?>
                        <?php foreach ($projectActivities as $event): ?>
                            <div class="history-activity-item">
        ...
        <div class="artwork-viewer" id="artwork-viewer">
            ...
            <div class="artwork-wrapper" id="artwork-wrapper">
                ...
                <?php elseif (preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $file['filename'])): ?>
                    <img src="uploads/projects/<?php echo $file['filename']; ?>" id="artwork-img" alt="Artwork">
                <?php else: ?>
                        ...
                        <div class="comment-item" data-id="<?php echo $comment['id']; ?>" data-type="<?php echo sanitize((string) ($comment['type'] ?? 'point')); ?>" data-comment-kind="<?php echo sanitize(annotationLabel($comment, $index + 1)); ?>">
                            ...
                            <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.75rem; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <div class="comment-actions">
                                    <span class="btn-reply-toggle" onclick="toggleReplyForm(<?php echo $comment['id']; ?>)">Reply</span>
                            ...
                            <!-- Reply Form -->
                            <div class="reply-input-box" id="reply-box-<?php echo $comment['id']; ?>">
                                ...
                                <button class="btn-save" style="width: 100%; padding: 0.4rem; font-size: 0.8rem; margin-top: 0.5rem;" onclick="saveReply(<?php echo $comment['id']; ?>)">Post Reply</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <script>
        ...
        window.filePath = 'uploads/projects/<?php echo $file['filename']; ?>';
        window.isPdf = <?php echo $file['file_type'] === 'pdf' ? 'true' : 'false'; ?>;
        window.initialComments = <?php echo json_encode($comments); ?>;

        function switchVersion(fileData) {
            const isLatest = fileData.is_latest;
            const imgPath = 'uploads/projects/' + fileData.filename;

            // Swap artwork image
            const existingImg = document.getElementById('artwork-img');
            if (existingImg) { existingImg.src = imgPath; }
