...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($client_name) || empty($_FILES['artwork']['name'])) {
        ...
        try {
            ...
            // Insert Project
            $stmt = $db->prepare("INSERT INTO projects (designer_id, title, job_name, client_name, token) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$_SESSION['user_id'], $title, $job_name, $client_name, $token]);
            ...
            if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
                // Insert File
                $stmt = $db->prepare("INSERT INTO files (project_id, filename, original_name, file_type, version) VALUES (?, ?, ?, ?, 1)");
