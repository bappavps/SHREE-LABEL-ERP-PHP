...
// Get Files
$stmt = $db->prepare("SELECT * FROM files WHERE project_id = ? ORDER BY version DESC");
$stmt->execute([$projectId]);
$files = $stmt->fetchAll();
...
// Get Activity
$stmt = $db->prepare("SELECT * FROM activity_log WHERE project_id = ? ORDER BY created_at DESC LIMIT 80");
$stmt->execute([$projectId]);
...
$comments = [];
if ($latestFileId > 0) {
    $commentStmt = $db->prepare("SELECT * FROM comments WHERE file_id = ? AND parent_id IS NULL ORDER BY created_at DESC LIMIT 20");
...
$reviewLink = BASE_URL . "/review.php?token=" . $project['token'];
$finalFileStmt = $db->prepare("SELECT * FROM files WHERE project_id = ? AND is_final = 1 ORDER BY version DESC LIMIT 1");
...
<div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
    ...
    <div class="right-col">
        ...
        <div class="glass-card" style="background: white; padding: 1.5rem; border-radius: 20px; box-shadow: var(--shadow-soft); margin-bottom: 2rem;">
            ...
            <div class="timeline" style="border-left: 2px solid #f1f5f9; padding-left: 1.5rem; margin-left: 0.5rem;">
...
<div id="portal-overlay" aria-hidden="true">
    <div id="portal-topbar">
        ...
        <div style="display:flex;align-items:center;gap:0.5rem;">
            <a href="<?php echo $reviewLink; ?>" target="_blank" title="Open in new tab"><i class="fas fa-external-link-alt"></i> Open in new tab</a>
...
function openPortal() {
    portalIframe.src = '<?php echo $reviewLink; ?>';
...
const shareText = "Hello! Please review the artwork for '<?php echo $project['title']; ?>' here: <?php echo $reviewLink; ?>";
...
function shareEmail() {
    const subject = encodeURIComponent("Artwork for Review: <?php echo $project['title']; ?>");
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(() => {
        notify('Review link copied to clipboard!');
    }).catch(() => {
        notify('Could not copy automatically. Please copy manually.');
    });
}
