...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare("SELECT token FROM projects WHERE id = ?");
    ...
    if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
        try {
            ...
            // Insert File
            $stmt = $db->prepare("INSERT INTO files (project_id, filename, original_name, file_type, version) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$projectId, $filename, $file['name'], $ext, $version]);
            
            // Update Project Status
            $stmt = $db->prepare("UPDATE projects SET status = 'pending' WHERE id = ?");
            $stmt->execute([$projectId]);
