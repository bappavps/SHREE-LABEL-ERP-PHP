...
try {
    ...
    $stmt = $db->prepare('SELECT id, filename, is_final FROM files WHERE id = ? AND project_id = ? LIMIT 1');
    ...
    $db->prepare('UPDATE files SET is_final = 0 WHERE id = ?')->execute([$fileId]);
    $db->prepare("UPDATE projects SET status = 'changes' WHERE id = ?")->execute([$projectId]);
    $db->prepare('INSERT INTO activity_log (project_id, action) VALUES (?, ?)')->execute([$projectId, 'Admin deleted final approval file.']);
