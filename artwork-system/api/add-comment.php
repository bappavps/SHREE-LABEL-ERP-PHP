...
function ensureCommentSchema(PDO $db): void {
    $requiredColumns = [
        'type' => "ALTER TABLE artwork_comments ADD COLUMN type VARCHAR(32) DEFAULT 'point' AFTER comment",
        'width' => 'ALTER TABLE artwork_comments ADD COLUMN width DECIMAL(6,2) DEFAULT NULL AFTER y_pos',
        'height' => 'ALTER TABLE artwork_comments ADD COLUMN height DECIMAL(6,2) DEFAULT NULL AFTER width',
        'drawing_data' => 'ALTER TABLE artwork_comments ADD COLUMN drawing_data LONGTEXT DEFAULT NULL AFTER height',
        'attachment' => 'ALTER TABLE artwork_comments ADD COLUMN attachment VARCHAR(255) DEFAULT NULL AFTER drawing_data'
    ];

    foreach ($requiredColumns as $column => $sql) {
        if (!hasColumn($db, 'artwork_comments', $column)) {
            $db->exec($sql);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (hasColumn($db, 'artwork_comments', 'type')) {
            $columns[] = 'type';
            $values[] = $type;
        }
        if (hasColumn($db, 'artwork_comments', 'x_pos')) {
            $columns[] = 'x_pos';
            $values[] = $xPos;
        }
        if (hasColumn($db, 'artwork_comments', 'y_pos')) {
            $columns[] = 'y_pos';
            $values[] = $yPos;
        }
        if (hasColumn($db, 'artwork_comments', 'width')) {
            $columns[] = 'width';
            $values[] = $width;
        }
        if (hasColumn($db, 'artwork_comments', 'height')) {
            $columns[] = 'height';
            $values[] = $height;
        }
        if (hasColumn($db, 'artwork_comments', 'drawing_data')) {
            $columns[] = 'drawing_data';
            $values[] = $drawingData;
        }
        if (hasColumn($db, 'artwork_comments', 'attachment')) {
            $columns[] = 'attachment';
            $values[] = $attachment;
        }
        if (hasColumn($db, 'artwork_comments', 'parent_id')) {
            $columns[] = 'parent_id';
            $values[] = $parentId;
        }

        $placeholders = implode(', ', array_fill(0, count($columns), '?'));
        $sql = 'INSERT INTO artwork_comments (' . implode(', ', $columns) . ') VALUES (' . $placeholders . ')';
        $stmt = $db->prepare($sql);
        $stmt->execute($values);
        
        // Get Project ID for notification and activity
        $stmt = $db->prepare("SELECT project_id FROM artwork_files WHERE id = ?");
        $stmt->execute([$fileId]);
        ...
        $stmt = $db->prepare("SELECT designer_id FROM artwork_projects WHERE id = ?");
