  ...
  <form class="pk-filters" method="get">
    ...
    <div>
      ...
      <select name="status" style="height:38px;border:1px solid #cbd5e1;border-radius:9px;padding:0 10px;font-size:.82rem;width:100%;background:#fff;">
        ...
        <option value="Finished Label" <?= $status === 'Finished Label' ? 'selected' : '' ?>>Finished Label</option>
  ...
  <div class="pk-tabs" id="pkTabs">
    <?php foreach ($allowedTabs as $tabKey): ?>
      ...
      <a
        ...
        href="<?= e(BASE_URL . '/modules/packing/index.php?tab=' . urlencode($tabKey)) ?>"
        data-tab="<?= e($tabKey) ?>"
        data-theme="<?= e($tabKey) ?>"
...
(function() {
  ...
  var pkCartonStatusMap = {};
  var pkCartonStatusLoaded = false;
  var pkCartonStatusPromise = null;
  var pkDefaultCartonSizes = ['57x15', '57x25', '78x25', '75mm', 'Barcode', 'Medicine'];
  var pkCartonSizeOptions = pkDefaultCartonSizes.slice();
  ...
  function pkNormCartonKey(v) {
    return String(v || '').toLowerCase().replace(/\s+/g, '');
  }

  function pkSetCartonSizeOptionsFromRows(rows) {
    var seen = {};
    var next = [];

    (rows || []).forEach(function(row) {
      var rawSize = String((row && row.size) || '').trim();
      var key = pkNormCartonKey(rawSize);
      if (!key || seen[key]) return;
      seen[key] = true;
      next.push(rawSize);
    });

    pkDefaultCartonSizes.forEach(function(sizeText) {
      var key = pkNormCartonKey(sizeText);
      if (!key || seen[key]) return;
      seen[key] = true;
      next.push(sizeText);
    });

    pkCartonSizeOptions = next.length ? next : pkDefaultCartonSizes.slice();
  }

  function pkGetCartonSizeOptions() {
    return (pkCartonSizeOptions && pkCartonSizeOptions.length ? pkCartonSizeOptions : pkDefaultCartonSizes).slice();
  }

  function pkCartonStatus(sizeText) {
    var key = pkNormCartonKey(sizeText);
    if (!key || !Object.prototype.hasOwnProperty.call(pkCartonStatusMap, key)) {
      return { qty: null, min_qty: 0, is_low: false };
    }
    return pkCartonStatusMap[key];
  }

  function pkCartonBadgeHtml(sizeText) {
    var st = pkCartonStatus(sizeText);
  ...
  function pkLoadCartonStatus(forceReload) {
    if (!forceReload && pkCartonStatusLoaded) {
      return Promise.resolve(pkCartonStatusMap);
    }
    if (pkCartonStatusPromise) {
      return pkCartonStatusPromise;
    }
    pkCartonStatusPromise = fetch(baseUrl + '/modules/packing/api.php?action=get_carton_stock_status', { credentials: 'same-origin' })
      .then(function(r) { return r.json(); })
      .then(function(res) {
        var nextMap = {};
        var rows = (res && Array.isArray(res.rows)) ? res.rows : [];
        pkSetCartonSizeOptionsFromRows(rows);
        rows.forEach(function(row) {
          var rawSize = String((row && row.size) || '').trim();
          var key = pkNormCartonKey(rawSize);
          if (!key) return;
        ...
        pkCartonStatusMap = nextMap;
        pkCartonStatusLoaded = true;
        return pkCartonStatusMap;
      })
      .catch(function() {
        return pkCartonStatusMap;
      })
      .finally(function() {
        pkCartonStatusPromise = null;
      });
    return pkCartonStatusPromise;
  }
  ...
  function printJobDetails() {
    ...
    var statusText = (statusLower === 'finished label') ? 'Finished Label' : ((statusLower === 'finished barcode') ? 'Finished Barcode' : ((statusLower === 'finished production') ? 'Finished Production' : ((statusLower === 'dispatched') ? 'Dispatched' : ((statusLower === 'packed' || statusLower === 'packing done') ? 'Packed' : 'Packing'))));
    ...
    var printHtml = '<!DOCTYPE html>' +
      ...
      '      <div style="margin-top:4px;text-align:right"><span class="a4-badge ' + ((statusLower === 'packing done' || statusLower === 'packed' || statusLower === 'dispatched' || statusLower === 'finished production' || statusLower === 'finished barcode' || statusLower === 'finished label' || statusLower === 'completed' || statusLower === 'closed' || statusLower === 'finalized' || statusLower === 'qc passed') ? 'ok' : '') + '">' + escHtml(statusText) + '</span></div>' +
      '    </div>' +
      '  </div>' +
  ...
  function fillModal(job) {
    ...
    var tabKeyNorm = String(job.packing_tab || activeTab || '').toLowerCase();
    var isBarcodeMode = tabKeyNorm === 'barcode' || tabKeyNorm === 'printing_label';
    var isPosRollTab = tabKeyNorm === 'pos_roll';
    ...
    Object.keys(operatorRollOverrides).forEach(function(k) {
      ...
      if (isBarcodeMode) {
        ...
        ['rps', 'rpc', 'cartons', 'extra', 'csize'].forEach(function(field) {
          ...
          if (Object.prototype.hasOwnProperty.call(src, 'csize_text') && String(src.csize_text || '').trim() !== '') {
            state.csize_text = String(src.csize_text || '').trim();
          }
      }
    ...
    function readBprFromSource(src) {
      ...
      for (var i = 0; i < keys.length; i++) {
        var n = Math.floor(toNum(src[keys[i]]));
        if (n > 0) return n;
      }
      return 0;
    }
    ...
    modalCardCanvas.innerHTML = '' +
      ...
      '  </div>' +
          ...
          + (isTerminal ? ('<span id="pkPackingDoneBadge" style="margin-left:8px;background:#22c55e;color:#fff;font-size:.72rem;font-weight:800;padding:3px 8px;border-radius:999px;">' + (isFinishedBarcode ? finishLabelText : (isFinishedProduction ? 'Finished Production' : (isDispatched ? 'Dispatched' : 'Packed'))) + '</span>') : '')
          + '    </h5>'
          ...
          + '        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px 12px;align-items:end;">'
            ...
            + '          </div>'
            + '          <div style="display:flex;flex-direction:column;gap:4px;">'
            + '            <label style="font-size:.85em;font-weight:700;color:#64748b;">Labels In 1 Roll</label>'
          ...
          + '      </div>'
          + '      <div id="pkSectionBMessage" style="min-height:20px;font-size:.82rem;color:#475569;font-weight:700;">' + (sectionBLocked ? ('Locked: ' + (isFinishedBarcode ? finishLabelText : (isFinishedProduction ? 'Finished Production' : (isDispatched ? 'Dispatched' : 'Packed'))) + ' (non-admin view)') : '') + '</div>'
          + '    </div>'
          + '  </div>'
              ...
              setTimeout(function() {
                ...
                function renderHelperCards(selectedRolls) {
                  ...
                  helperCardsWrap.innerHTML = selectedRolls.map(function(roll, idx) {
                    ...
                    if (isBarcodeMode) {
                      ...
                      var csizeBadge = pkCartonBadgeHtml(csizeText);
                      ...
                      return ''
                        ...
                        + '    <div><label style="display:block;font-size:.68rem;font-weight:700;color:#64748b">Labels In 1 Roll (bpr)</label><input type="number" class="pk-bc-per-roll" data-roll-key="' + escHtml(key) + '" min="1" step="1" value="' + String(bpr) + '" style="width:100%;padding:5px 6px;border:1px solid #cbd5e1;border-radius:6px"></div>'
                        ...
                        + '    <div style="grid-column:span 2"><label style="display:block;font-size:.68rem;font-weight:700;color:#64748b">Carton size ' + csizeBadge + '</label><select class="pk-bc-csize" data-roll-key="' + escHtml(key) + '" style="width:100%;padding:5px 6px;border:1px solid #cbd5e1;border-radius:6px;background:#f8fafc">' + barcodeCartonOptions.map(function(opt){var sel=(normCsizeText(opt)===normCsizeText(csizeText)?' selected':'');return '<option value="'+escHtml(opt)+'"'+sel+'>'+escHtml(opt)+'</option>';}).join('') + '</select></div>'
                        + '  </div>'
                    ...
                    var nonBarcodeCartonOptions = pkGetCartonSizeOptions();
                    ...
                    var csizeBadgeNonBarcode = pkCartonBadgeHtml(csizeNow);
                    return ''
                      ...
                      + '    <div><label style="display:block;font-size:.68rem;font-weight:700;color:#64748b">Carton size (mm) ' + csizeBadgeNonBarcode + '</label><select class="pk-roll-csize" data-roll-key="' + escHtml(key) + '" style="width:100%;padding:5px 6px;border:1px solid #cbd5e1;border-radius:6px;background:#f8fafc">' + nonBarcodeCartonOptions.map(function(s){var sel=(normCsizeText(s)===normCsizeText(csizeNow)?' selected':'');return '<option value="'+escHtml(s)+'"'+sel+'>'+escHtml(s)+'</option>';}).join('') + '</select></div>'
                ...
                function bindHelperCardEvents() {
                  ...
                  Array.prototype.slice.call(helperCardsWrap.querySelectorAll('.pk-roll-extra')).forEach(function(node) {
                    ...
                    Array.prototype.slice.call(helperCardsWrap.querySelectorAll('.pk-roll-csize')).forEach(function(node) {
                      node.addEventListener('change', function() {
                        var key = String(node.getAttribute('data-roll-key') || '').trim();
                        if (!key) return;
                        if (!rollHelperOverrides[key] || typeof rollHelperOverrides[key] !== 'object') rollHelperOverrides[key] = {};
                        rollHelperOverrides[key].csize_text = String(node.value || '').trim();
                        recalc();
                      });
                    });
                }
                ...
                // Initial calculation after a fresh carton status fetch (avoid stale same-session cache).
                pkLoadCartonStatus(true).finally(function() {
                  recalc();
                });
              }, 10);
          + imageHtml
    ...
    setTimeout(function() {
      ...
      function ensurePackingDoneBadge() {
        ...
        badge.textContent = isFinishedBarcodeNow ? finishStatusDoneText : (isFinishedProductionNow ? 'Finished Production' : (isDispatchedNow ? 'Dispatched' : 'Packed'));
      ...
      function applySectionBLockState() {
        ...
        // Always restore roll-select checkboxes to enabled so manager can change roll selection
        var rollSelectFields = section.querySelectorAll('.pk-jc-section-content-b .pk-roll-select');
        rollSelectFields.forEach(function(cb) {
        ...
        if (isTerminalNow && !isAdminUser) {
          setSectionMessage('Locked: ' + (isFinishedBarcodeNow ? finishStatusDoneText : (isFinishedProductionNow ? 'Finished Production' : (isDispatchedNow ? 'Dispatched' : 'Packed'))) + ' (non-admin view)', false);
          ...
          setSectionMessage((isFinishedBarcodeNow ? finishStatusDoneText : (isFinishedProductionNow ? 'Finished Production' : (isDispatchedNow ? 'Dispatched' : 'Packed'))) + ': admin override enabled for edits.', false);
        } else if (operatorSubmittedNow) {
        ...
        // Always re-evaluate print button states after lock state changes
        updateStickerButtonState();
        updateLabelButtonState();
      }
      ...
      // Re-evaluate sticker/label button state whenever manager changes roll selection.
      // Must be set up here (outer setTimeout scope) where updateStickerButtonState is accessible.
      var rollCbsForPrint = section.querySelectorAll('.pk-jc-section-content-b .pk-roll-select');
      rollCbsForPrint.forEach(function(cb) {
          ...
          if (buttonAction === 'finished') {
            ...
            fetch(baseUrl + '/modules/packing/api.php', {
              ...
              .then(function(resp) {
                ...
                applySectionBLockState();
                setSectionMessage('Status updated: Packed ✓ ' + (isPaperRollTypeTab ? 'Finished Production is now enabled.' : (isBarcodeMode ? (finishStatusDoneText + ' is now enabled.') : 'You can now mark Dispatched.')) , false);
              })
              .catch(function() {
            ...
            setSectionMessage('Updating status to ' + finishStatusDoneText + '...', false);

            var fdFinishedBarcode = new FormData();
            fdFinishedBarcode.append('action', tabKeyNorm === 'printing_label' ? 'mark_finished_label' : 'mark_finished_barcode');
            ...
            fetch(baseUrl + '/modules/packing/api.php', {
              ...
              .then(function(resp) {
                if (!resp || !resp.ok) {
                  setSectionMessage((resp && resp.message) ? resp.message : (finishStatusDoneText + ' failed.'), true);
                  isFinishedBarcodeNow = false;
                  isTerminalNow = isDispatchedNow || isPackedNow || isFinishedProductionNow || isFinishedBarcodeNow;
                  return;
                }
                ...
                isFinishedProductionNow = false;
                isTerminalNow = true;
                modalStatusNow = finishStatusDoneNorm;
                job.status = finishStatusDoneText;
                applySectionBLockState();
                setSectionMessage(finishStatusDoneText + ' completed ✓ Moving to History...', false);
              ...
              .catch(function() {
                setSectionMessage(finishStatusDoneText + ' failed.', true);
                isFinishedBarcodeNow = false;
                isTerminalNow = isDispatchedNow || isPackedNow || isFinishedProductionNow || isFinishedBarcodeNow;
              })
              .finally(function() {
                if (finishedBarcodeBtn) {
                  ...
                  finishedBarcodeBtn.textContent = finishStatusDoneText;
                  applySectionBLockState();
                }
              });
  ...
  document.addEventListener('click', function(e) {
    ...
    var btn = target.closest('.pk-open-modal');
    if (!btn) return;
  ...
  var printDetailsBtn = document.getElementById('pkPrintDetailsBtn');
  if (printDetailsBtn) {
