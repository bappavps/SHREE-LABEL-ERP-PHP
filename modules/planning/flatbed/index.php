...
$pageTitle   = 'Die-Cutting — Job Planning';
$db          = getDB();
...
// ── Die-cutting department filter (same as job card page) ──
$dcWhere = "(
...
$currentUrl = BASE_URL . '/modules/planning/flatbed/index.php';
$planningPageKey = 'die-cutting';
...
<div class="dc-header no-print">
  <div>
    <h1><i class="bi bi-crop"></i> Die-Cutting — Job Planning</h1>
    <div style="font-size:.75rem;color:#64748b;font-weight:600">All Die-Cutting (FlatBed) job card details &middot; Filter by date range &middot; <?= $dateLabels[$dateFilter] ?></div>
  </div>
...
<div class="dc-stats no-print">
  <div class="dc-stat">
    <div class="dc-stat-icon" style="background:#f0fdfa;color:var(--dc-brand)"><i class="bi bi-crop"></i></div>
...
<div class="card">
  <div class="card-header no-print" style="display:flex;justify-content:space-between;align-items:center">
    <span class="card-title"><i class="bi bi-table"></i> Die-Cutting Jobs — <?= htmlspecialchars($dateLabels[$dateFilter]) ?></span>
  </div>
  <!-- Print-only header -->
  <div style="display:none" class="print-only-header">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;border-bottom:2px solid #0ea5a4;padding-bottom:10px">
      ...
      <div style="text-align:right">
        <div style="font-weight:900;font-size:1rem;color:#0ea5a4">Die-Cutting — Job Planning</div>
  ...
  <div style="overflow:auto">
    <table class="dc-tbl" id="dcTable">
      ...
      <tbody id="dcTbody">
      <?php if (empty($rows)): ?>
        <tr><td colspan="21" style="padding:24px;text-align:center;color:#94a3b8"><i class="bi bi-inbox" style="font-size:2rem;opacity:.3;display:block;margin-bottom:8px"></i>No Die-Cutting jobs found for this period.</td></tr>
      <?php else: ?>
        ...
        <tr data-search="<?= htmlspecialchars($searchStr, ENT_QUOTES) ?>">
          ...
          <td class="dc-num" style="color:#0ea5a4"><?= htmlspecialchars($r['dc_total_qty'] ?: '—') ?></td>
          ...
          <td><span class="dc-badge" style="<?= e($stsStyle) ?>"><?= htmlspecialchars($sts) ?></span></td>
