...
$pageTitle = $isOperatorView ? 'Label Slitting Operator' : 'Lable Sltiing Job Cards';
$db = getDB();
...
$jobsSql = "
    ...
    LEFT JOIN jobs greatgrandprev ON grandprev.previous_job_id = greatgrandprev.id
    WHERE {$dcWhereClause}
...
foreach ($jobs as &$job) {
  ...
  $job['planning_repeat_gap'] = (string)($planningExtra['repeat_gap'] ?? ($planningExtra['repeat'] ?? ''));
    ...
    if ($rawNotes !== '' && stripos($rawNotes, 'Die-cutting queued from Flexo') === 0) {
      ...
      $jumboRef = trim((string)($job['grandprev_job_no'] ?? ''));
    ...
    } elseif ($rawNotes !== '' && (stripos($rawNotes, 'Label slitting queued from upstream') === 0 || stripos($rawNotes, 'Label slitting released from') === 0)) {
      $planRef = trim((string)($job['planning_ref_no'] ?? ''));
      if ($planRef === '' && preg_match('/\|\s*Plan:\s*([^|\n]+)/i', $rawNotes, $m)) {
        $planRef = trim((string)($m[1] ?? ''));
      }

      $prevDept = strtolower(trim((string)($job['prev_department'] ?? '')));
      $grandPrevDept = strtolower(trim((string)($job['grandprev_department'] ?? '')));
      $greatGrandPrevDept = strtolower(trim((string)($job['greatgrandprev_department'] ?? '')));

      $labelRef = trim((string)($job['job_no'] ?? ''));
      $dcRef = '';
      $flexoRef = '';
      $jumboRef = '';

      if (in_array($prevDept, ['flatbed', 'die-cutting', 'die_cutting', 'die cutting'], true)) {
        $dcRef = trim((string)($job['prev_job_no'] ?? ''));
        $flexoRef = trim((string)($job['grandprev_job_no'] ?? ''));
        $jumboRef = trim((string)($job['greatgrandprev_job_no'] ?? ''));
      } else {
        $flexoRef = trim((string)($job['prev_job_no'] ?? ''));
        $jumboRef = trim((string)($job['grandprev_job_no'] ?? ''));
        if ($jumboRef === '' && $grandPrevDept === 'jumbo_slitting') {
          $jumboRef = trim((string)($job['grandprev_job_no'] ?? ''));
        }
        if ($jumboRef === '' && $greatGrandPrevDept === 'jumbo_slitting') {
          $jumboRef = trim((string)($job['greatgrandprev_job_no'] ?? ''));
        }
      }

      $displayName = trim((string)($job['display_job_name'] ?? ''));
      $normalized = 'Label slitting queued from upstream'
        . ' | Plan: ' . ($planRef !== '' ? $planRef : 'N/A')
        . ' | Jumbo: ' . ($jumboRef !== '' ? $jumboRef : 'N/A')
        . ' | Flexo: ' . ($flexoRef !== '' ? $flexoRef : 'N/A')
        . ' | Die-Cut: ' . ($dcRef !== '' ? $dcRef : 'N/A')
        . ' | Label: ' . ($labelRef !== '' ? $labelRef : 'N/A');
      if ($displayName !== '') {
        $normalized .= ' | Job name: ' . $displayName;
      }
      $job['notes_display'] = $normalized;
...
<div class="breadcrumb">
  ...
  <?php if ($isOperatorView): ?>
    ...
    <span>Label Slitting Operator</span>
  <?php else: ?>
    <span>Production</span><span class="breadcrumb-sep">&#8250;</span>
    <span>Job Cards</span><span class="breadcrumb-sep">&#8250;</span>
    <span>Lable Sltiing</span>
  <?php endif; ?>
</div>

<style>
:root { --dc-brand: #f97316; --dc-brand-light: #ffedd5; --dc-brand-dark: #c2410c; --dc-blue: #3b82f6; }
...
.dc-stat:hover{border-color:var(--dc-brand);box-shadow:0 4px 12px rgba(249,115,22,.12)}
.dc-stat.active{border-color:var(--dc-brand);box-shadow:0 4px 16px rgba(249,115,22,.18)}
...
.dc-card:hover{border-color:var(--dc-brand);box-shadow:0 4px 16px rgba(249,115,22,.1)}
...
/* ── Bulk Bar ── */
.dc-bulk-bar{display:none;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;border-radius:14px;padding:14px 20px;margin-bottom:14px;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;box-shadow:0 4px 16px rgba(249,115,22,.25)}
...
.ht-bulk-bar{display:none;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;border-radius:12px;padding:12px 20px;margin-bottom:12px;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;box-shadow:0 4px 16px rgba(249,115,22,.25)}
...
<div class="dc-header no-print" style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:16px">
  <div>
    <h1 style="margin:0;font-size:1.3rem;font-weight:900;color:#0f172a;display:flex;align-items:center;gap:8px">
      <i class="bi bi-layout-split"></i> <?= $isOperatorView ? 'Label Slitting Operator' : 'Lable Sltiing Job Cards' ?>
    </h1>
    <div style="font-size:.72rem;color:#94a3b8;margin-top:4px;font-weight:600">
      Auto-generated for label slitting after Flatbed or Flexo printing &middot; Sequential gating from previous department
    </div>
  </div>
...
<?php if (empty($activeJobs) && empty($historyJobs)): ?>
  <div class="dc-empty">
    ...
    <p>No active label slitting jobs.</p>
  </div>
<?php else: ?>
  ...
  <div class="dc-card <?= $isQueued ? 'dc-queued' : '' ?>" data-status="<?= e($sts) ?>" data-lockstate="<?= $isLocked ? 'locked' : 'unlocked' ?>" data-search="<?= e($searchText) ?>" data-id="<?= $job['id'] ?>" onclick="openJobDetail(<?= $job['id'] ?>)">
    ...
    <div class="dc-card-head">
      <div class="dc-jobno"><i class="bi bi-layout-split"></i> <?= e($job['job_no']) ?></div>
...
<div class="ht-table-wrap">
  <table class="ht-table" id="htTable">
    ...
    <?php if (empty($historyJobs)): ?>
      <tr><td colspan="12" style="padding:40px;text-align:center;color:#94a3b8"><i class="bi bi-inbox" style="font-size:2rem;opacity:.3"></i><br>No finished label slitting jobs found</td></tr>
    <?php else: ?>
...
function calcLabelSlittingWastage(form, prevProduction) {
  if (!form) return;
  const prodInput = form.querySelector('[name=label_slitting_total_production]');
  const wastageInput = form.querySelector('[name=label_slitting_wastage_percentage]');
  if (!prodInput || !wastageInput) return;
  const prev = Number(prevProduction || 0);
  const cur = Number(prodInput.value || 0);
  if (prev > 0 && cur > 0) {
...
function htBulkPrint() {
  ...
  (async function() {
    ...
    w.document.write('<!DOCTYPE html><html><head><title>History Bulk Print - ' + jobs.length + ' Lable Sltiing Job Cards</title><style>@page{margin:12mm}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}</style></head><body>' + pages + '</body></html>');
...
async function submitAndClose(id) {
  ...
  if (!extraData.label_slitting_qty_in_roll || !extraData.label_slitting_total_roll || !extraData.label_slitting_total_production) {
    alert('Quantity in roll, Total Roll and Total Production are required.');
    return;
  }
...
async function openJobDetail(id, mode) {
  ...
  _lastPhotoPath = String(extra.label_slitting_photo_path || extra.die_cutting_photo_path || '');
  _lastVoicePath = String(extra.label_slitting_voice_note_path || extra.die_cutting_voice_note_path || '');
  ...
  html += `<div class="dc-op-section" style="border-color:#99f6e4"><div class="dc-op-b" style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;background:linear-gradient(135deg,#f0fdfa,#fff)">
    ...
    <div style="text-align:right">
      <div style="font-size:.72rem;font-weight:800;text-transform:uppercase;color:#334155">Lable Sltiing Job Card</div>
  ...
  if (job.prev_job_no) {
    ...
    html += `<div class="dc-op-section" style="padding:12px;background:${prevDone?'#f0fdf4':'#fef3c7'};border-radius:10px;border-left:4px solid ${pvColor}">
      <div style="display:flex;align-items:center;gap:8px;font-size:.78rem;font-weight:700">
        ...
        Previous Job: <span style="color:var(--dc-brand)">${esc(job.prev_job_no)}</span>
  ...
  // ── Printing Production vs Planning Quantity ──
  {
    ...
    if (hasPlanQty || hasPrintQty) {
      ...
      html += `<div class="dc-op-section" style="border-color:#e0e7ff">
        <div class="dc-op-h" style="background:#eef2ff;color:#4338ca"><i class="bi bi-bar-chart-line"></i> Production from Flatbed or Printing Production</div>
        <div class="dc-op-b">
  ...
  // ── Order Details ──
  html += `<div class="dc-op-section"><div class="dc-op-h"><i class="bi bi-layout-split"></i> Order Details</div><div class="dc-op-b dc-op-grid-2">
    ...
    <div class="dc-op-field"><label>Production from Flatbed or Printing Production</label><div class="fv" style="color:#3b82f6;font-weight:900">${esc(job.prev_actual_qty || '—')}</div></div>
  ...
  {
    const qtyInRoll = extra.label_slitting_qty_in_roll || '';
  ...
  // ── Photo Upload ──
  {
    const existingPhoto = extra.label_slitting_photo_url || extra.die_cutting_photo_url || '';
...
function renderDCPrintCardHtml(job, qrDataUrl) {
  ...
  const previewHtml = previewUrl
    ? `<tr><td colspan="4" style="padding:0"><div style="font-size:.66rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;color:#c2410c;background:#fff7ed;padding:6px 8px;border-radius:4px;margin:8px 0 4px">Job Preview</div><div style="margin-bottom:6px"><img src="${esc(previewUrl)}" style="max-width:300px;max-height:180px;border-radius:8px;border:1px solid #e2e8f0"></div></td></tr>`
    : '';

  return `<div style="font-family:'Segoe UI',Arial,sans-serif;padding:20px;max-width:760px;margin:0 auto;color:#0f172a">
    <div style="border:2px solid #c2410c;border-radius:12px;overflow:hidden">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;padding:12px 14px;background:#fff7ed;border-bottom:2px solid #c2410c">
        <div>
        ...
        <div style="display:flex;align-items:flex-start">
          <div style="text-align:right">
            ...
            <div style="font-size:.92rem;font-weight:900;color:#c2410c">Lable Sltiing</div>
      ...
      <div style="padding:10px 12px">
        ...
        ${(() => {
          ...
          const pctText = canCompare && planQty > 0
            ? (((diff / planQty) * 100) > 0 ? '+' : '') + ((diff / planQty) * 100).toFixed(1) + '%'
            : '';
          return `<div style="font-size:.66rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;color:#4338ca;background:#eef2ff;padding:5px 8px;border-radius:4px;margin-top:8px">Production from Flatbed or Printing Production</div>
          <table style="width:100%;border-collapse:collapse;font-size:.72rem;margin-bottom:10px">
            <tr>
        ...
        <div style="font-size:.66rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;color:#c2410c;background:#fff7ed;padding:5px 8px;border-radius:4px;margin-top:8px">Execution Summary</div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;padding:10px 12px;border-top:2px solid #c2410c;background:#fff7ed">
      ...
      <div style="padding:6px 12px;font-size:.58rem;color:#64748b;display:flex;justify-content:space-between;border-top:1px solid #fdba74">
        <span>Document: Lable Sltiing Job Card | ${esc(COMPANY.name || '')}</span>
...
function dcBulkPrint() {
  ...
  (async function() {
    ...
    const w = window.open('', '_blank', 'width=820,height=920');
    w.document.write('<!DOCTYPE html><html><head><title>Bulk Print - ' + jobs.length + ' Lable Sltiing Job Cards</title><style>@page{margin:12mm}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}</style></head><body>' + pages + '</body></html>');
    w.document.close(); w.focus(); setTimeout(() => w.print(), 400);
  })();
}
