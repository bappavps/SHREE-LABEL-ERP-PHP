...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE customers SET name=?,company=?,email=?,phone=?,address=?,city=?,state=?,pincode=?,gstin=? WHERE id=?");
        $stmt->bind_param('sssssssssi',
            $data['name'], $data['company'], $data['email'], $data['phone'],
            $data['address'], $data['city'], $data['state'], $data['pincode'], $data['gstin'], $id
        );
        if ($stmt->execute()) {
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <h4 class="fw-bold mb-0"><i class="fas fa-edit me-2 text-primary"></i>Edit Customer</h4>
