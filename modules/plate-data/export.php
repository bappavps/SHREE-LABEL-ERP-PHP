...
<Worksheet ss:Name="<?= pdXmlEsc($moduleLabel) ?>">
<Table>
  ...
  <Row ss:Height="28"><?= pdMergeCell($companyName, $mergeCount, 's_company') ?></Row>
  <?php if ($companyTagline !== ''): ?><Row ss:Height="18"><?= pdMergeCell($companyTagline, $mergeCount, 's_tagline') ?></Row><?php endif; ?>
  <?php if ($companyAddress !== ''): ?><Row ss:Height="16"><?= pdMergeCell($companyAddress, $mergeCount, 's_detail') ?></Row><?php endif; ?>
  <?php
    $contactParts = [];
    if ($companyPhone !== '') $contactParts[] = 'Ph: ' . $companyPhone;
    if ($companyEmail !== '') $contactParts[] = $companyEmail;
  ?>
  <?php if ($contactParts): ?><Row ss:Height="16"><?= pdMergeCell(implode('  |  ', $contactParts), $mergeCount, 's_detail') ?></Row><?php endif; ?>

  <Row ss:Height="8"><Cell ss:StyleID="s_body"/></Row>
  <Row ss:Height="26"><?= pdMergeCell(strtoupper($moduleLabel) . ' REPORT — ' . date('d M Y, h:i A') . '  |  Total: ' . count($rows) . ' records', $mergeCount, 's_title') ?></Row>
  <Row ss:Height="6"><Cell ss:StyleID="s_body"/></Row>

  <Row ss:Height="24">
    <Cell ss:StyleID="s_header"><Data ss:Type="String">SL No.</Data></Cell>
    <?php foreach ($columns as $label): ?>
      <Cell ss:StyleID="s_header"><Data ss:Type="String"><?= pdXmlEsc($label) ?></Data></Cell>
    <?php endforeach; ?>
  </Row>

  <?php if (!$rows): ?>
    <Row ss:Height="22">
      <?= pdMergeCell('No records found.', $mergeCount, 's_body') ?>
    </Row>
  <?php else: ?>
    <?php foreach ($rows as $index => $row): ?>
      <?php
        ...
        if ($displaySlNo === '') $displaySlNo = (string)($index + 1);
      ?>
      <Row ss:Height="22">
        <?= pdCell($displaySlNo, $slStyle) ?>
        <?php foreach ($columns as $key => $label): ?>
          <?php $val = trim((string)($row[$key] ?? '')); ?>
          <?php if (in_array($key, $numericKeys, true) && $val !== '' && is_numeric($val)): ?>
            <?= pdNumCell($val, $numStyle) ?>
          <?php else: ?>
            <?= pdCell(dieToolingFormatDisplayNumber($val), $textStyle) ?>
          <?php endif; ?>
        <?php endforeach; ?>
      </Row>
    <?php endforeach; ?>
  ...
  <Row ss:Height="8"><Cell ss:StyleID="s_body"/></Row>
  <Row ss:Height="24"><?= pdMergeCell('SUMMARY', $mergeCount, 's_sum_title') ?></Row>
  <Row ss:Height="22">
    <?= pdMergeCell('Total Rows', max(0, $mergeCount - 1), 's_sum_label') ?>
    <?= pdNumCell(count($rows), 's_sum_val') ?>
  </Row>
  <Row ss:Height="22"><?= pdMergeCell('Generated on ' . $dateNow, $mergeCount, 's_footer') ?></Row>
</Table>
</Worksheet>
</Workbook>
  <?php
    ...
    <tbody>
      ...
      <?php else: ?>
        <?php foreach ($rows as $index => $row): ?>
          ...
          <tr>
            <td><?= e($displaySlNo !== '' ? $displaySlNo : (string)($index + 1)) ?></td>
            <?php foreach ($columns as $key => $label): ?>
