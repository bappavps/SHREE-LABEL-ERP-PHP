...
if (empty($_SESSION['setup_csrf_live'])) {
    $_SESSION['setup_csrf_live'] = bin2hex(random_bytes(32));
}
...
$checks = [
    ...
    ['setup lock not present', !file_exists($lockFile), file_exists($lockFile) ? 'data/install.lock exists' : 'No lock file'],
];

$pendingMigrationFiles = collect_migration_sql_files($migrationDir);
$pendingMigrationCount = count($pendingMigrationFiles);
$pendingMigrationPreview = array_slice(array_map('basename', $pendingMigrationFiles), 0, 6);

$pathChecks = [
    'config' => check_writable(__DIR__ . '/config'),
    'data' => check_writable(__DIR__ . '/data'),
    'uploads' => check_writable(__DIR__ . '/uploads'),
    'uploads/company' => check_writable(__DIR__ . '/uploads/company'),
    'uploads/library' => check_writable(__DIR__ . '/uploads/library'),
];
...
if (file_exists($lockFile)) {
    ...
    <head>
      ...
      <title>Live Installer Locked</title>
      <style>body{font-family:Arial,sans-serif;background:#f8fafc;padding:24px}.card{max-width:860px;margin:0 auto;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px}.warn{color:#92400e;background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:10px}.hint{margin-top:12px;color:#334155}</style>
    </head>
    <body>
      <div class="card">
        <h2>Live installer is locked</h2>
        <p>This ERP appears already installed on this server.</p>
        <p class="warn">Installed at: <?= h($installedAt !== '' ? $installedAt : 'unknown') ?></p>
        <p class="hint">Delete <strong>data/install.lock</strong> only if you really need a full reinstall and can lose existing live data.</p>
      </div>
    </body>
    </html>
    <?php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $submittedToken = (string)($_POST['setup_csrf_live'] ?? '');
    if (!hash_equals((string)$_SESSION['setup_csrf_live'], $submittedToken)) {
        ...
        if ($conn->connect_error) {
            ...
            if (empty($errors)) {
                ...
                if (file_put_contents($runtimeConfigFile, $runtimePhp, LOCK_EX) === false) {
                    ...
                    log_ok('Runtime config saved for live environment.');
                }
            }
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>ERP Live Fresh Installation</title>
<style>
body{font-family:Arial,sans-serif;background:#f5f6f8;padding:20px;margin:0}
.wrap{max-width:1020px;margin:0 auto;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px}
h1{margin:0 0 8px}.muted{color:#64748b;font-size:.9rem}
...
.check{display:flex;gap:8px;align-items:center;padding-top:8px}.check input{width:auto}
...
.ok{color:#166534;margin:6px 0}.err{color:#b91c1c;margin:6px 0}
...
@media (max-width:760px){.summary-grid{grid-template-columns:1fr 1fr}.grid{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="wrap">
  <h1>Enterprise ERP Live Fresh Installation</h1>
  <p class="muted">This installer is dedicated to live server deployment. It runs environment checks, imports schema, applies pending SQL migrations, creates default admin, writes live runtime config, and creates installation lock.</p>

  <div class="summary">
    <div class="summary-title">Live Setup Snapshot</div>
    <div class="summary-grid">
      <div class="summary-item">
        <div class="k">Target Environment</div>
      ...
      <div class="summary-item">
        <div class="k">Detected Base URL</div>
        <div class="v"><?= h($baseUrlDetected === '' ? '/' : $baseUrlDetected) ?></div>
      </div>
      <div class="summary-item">
  ...
  <form method="post">
    <input type="hidden" name="setup_csrf_live" value="<?= h((string)$_SESSION['setup_csrf_live']) ?>">
    <div class="grid">
      ...
      <div class="full"><label>Base URL (empty for domain root, or /folder-name)</label><input name="base_url" value="<?= h($input['base_url']) ?>" placeholder="/erp"></div>
      <div class="full check"><input type="checkbox" name="create_database" value="1" <?= $input['create_database'] === '1' ? 'checked' : '' ?>><label>Create database automatically (keep disabled on most shared hosting plans)</label></div>
    </div>
    <button class="btn" type="submit">Run Live Fresh Installation</button>
  </form>
  ...
  <?php if (!empty($log)): ?>
    ...
    <?php if (empty($errors)): ?>
      <div class="next">
        ...
        Next action: remove or rename this installer file and setup.php for security.
      </div>
    <?php endif; ?>
  <?php endif; ?>
</div>
